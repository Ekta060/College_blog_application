...
class CreateBlog(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # to give id to each blog id of one user uuid is used
...
def blog_details(request, slug):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            comment.user = request.user     #current user
            comment.blog = blog
            comment.save()
            return HttpResponseRedirect(reverse('App_Blog:details', kwargs={'slug':slug}))

    return render(request, 'App_Blog/blog_details.html', {'blog':blog, 'comment_form':comment_form, 'liked':liked})


def search(request):
    if request.method == "POST":
        searched = request.POST['searched']
        blogs = Blog.objects.filter(blog_title__contains=searched)
        return render(request, "App_Blog/search.html", {'searched':searched, 'blogs':blogs})
    else:
        return render(request, "App_Blog/search.html", {})



#pk is primary key of blog
@login_required
